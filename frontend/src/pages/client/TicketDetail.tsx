...
export default function ClientTicketDetail() {
  ...
  if (isLoading) return (
    <div className="p-6 flex flex-col gap-3">
      {[1,2,3].map(i => <div key={i} className="h-20 rounded-xl animate-pulse" style={{ background: '#1f2330' }} />)}
    </div>
  )
  if (!ticket) return <div className="p-6 text-center" style={{ color: '#8b92a5' }}>Ticket not found.</div>

  return (
    <div className="min-h-screen pb-24" style={{ background: '#0f1117' }}>
      {/* Sticky header */}
      <div className="sticky top-0 z-10 px-4 py-3 border-b flex items-center gap-3" style={{ background: '#181c24', borderColor: '#2e3545' }}>
      ...
      <div className="p-4 max-w-3xl mx-auto">
        <h1 className="text-base font-semibold mb-1" style={{ color: '#e8eaf0' }}>{ticket.title}</h1>
        <div className="flex items-center gap-3 mb-4">
          <PriorityDot priority={ticket.priority} />
          <span className="text-xs capitalize" style={{ color: '#8b92a5' }}>{ticket.category}</span>
          <span className="text-xs" style={{ color: '#565e72' }}>{timeStr(ticket.created_at)}</span>
        </div>
        ...
        {/* Activity timeline */}
        <h2 className="text-xs font-semibold uppercase tracking-wider mb-3" style={{ color: '#565e72' }}>Activity</h2>
              ...
              {a.activity_type === 'status_changed' ? (
                <div className="text-center py-2">
                  <span className="text-xs px-3 py-1 rounded-full" style={{ background: '#262c3a', color: '#8b92a5' }}>
                    {a.actor.name} moved to <strong style={{ color: '#e8eaf0' }}>{a.new_value?.replace('_',' ')}</strong> · {timeStr(a.created_at)}
                  </span>
                </div>
              ) : a.comment ? (
                <div className="rounded-xl p-3 border" style={{ background: '#1f2330', borderColor: '#2e3545' }}>
      ...
      {ticket.status !== 'closed' && (
        <div className="fixed bottom-0 left-0 right-0 p-4 border-t" style={{ background: '#181c24', borderColor: '#2e3545' }}>
